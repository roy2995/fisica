...
int main()
{
    RenderWindow window(VideoMode(640, 750), "Proyect glaiding");
    ...
    spr_fondo_1.setScale(Vector2f(2.7f, 4.0f));
    spr_fondo_2.setScale(Vector2f(2.5f, 4.0f));
    spr_player.setScale(Vector2f(2.0f, 2.0f));
    spr_floor.setScale(Vector2f(1.0f, 2.0f));

    //configuramos el cuerpo 
    world player;
    player.instansgravity(vect_2(0.0f, 450.0f));//gavedad del objeto
    //lo setea en el origen de la ventana
    cuerpo _player;
    _player.mass = 2.0f;
    _player.invmass = 1 / _player.mass;
    _player.position = vect_2(100.0f, 130.0f);
    ...
    spr_floor.setOrigin(Vector2f(10.0f, -1.5f));
    ...
    while (window.isOpen())
    {
        ...
        if (!boolSpace) {
            ...
            if (parachute && _player.position.y <= 430.0f) {
                _player.force.y = -990.0f;
                cout << "fuerza normal" << endl;
            }
            //esto lo hice por que la ultima vez mi personaje callo demaciado rapido asi que aplico mas fuerza para que ser note el gliding
            if (parachute && _player.position.y >= 430.0f) {
                _player.force.y = -1220.0f;
                cout << "fuerza superior" << endl;
            }
        }
        //caminar hacia la derecha
        if (walckD) {
            _player.force.x = 400.0f;
        }
